...
const TaskDash = () => {
  ...
  const lottieOptions = {
    loop: true,
    autoplay: true,
    animationData: loadingAnimation, // Replace with your JSON data
    rendererSettings: {
          ...
          <table className="min-w-full bg-white border border-gray-300" ref={tableRef}>
            ...
            <tbody>
              {tasks.length ? (
                tasks.map(task => (
                  <tr key={task?._id} className="hover:bg-gray-100">
                    ...
                    <td className="py-2 px-4 border-b *:mx-2 *:my-1 flex items-center *:cursor-pointer">
                      <button
                        onClick={() => updateTaskStatus(task?._id, 'completed', task?.status)}
                ...
                <tr>
                  <td colSpan={8} className='py-2 px-4 font-bold'>no data</td>
                </tr>
              )}
            </tbody>
          </table>
